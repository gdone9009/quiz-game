...
![Git 클론](docs/screenshots/gitclone.png)

```mermaid
graph TD
    %% 시작 및 데이터 로드
    Start((시작)) --> Load{state.json 확인}
    Load -- "파일 없음" --> Default[기본 데이터 생성]
    Load -- "파일 존재" --> Read[데이터 역직렬화/로드]
    Default --> Init[QuizGame 객체 초기화]
    Read --> Init
    ...
    %% 퀴즈 풀기 흐름
    Menu -- "1. 퀴즈 풀기" --> Play[퀴즈 실행 루프]
    Play --> Input[정답 입력 및 유효성 검사]
    Input --> Logic{정답 판별}
    Logic -- "정답/오답" --> Feed[설명 출력 및 점수 누적]
    Feed --> Play
    Play -- "모든 문제 완료" --> SaveScore[최고 점수 갱신 및 저장]
    SaveScore --> Menu

    %% 퀴즈 추가 흐름
    Menu -- "2. 퀴즈 추가" --> Add[새 퀴즈 입력]
    Add --> Valid{입력 유효성 검사}
    Valid -- "통과" --> SaveQuiz[JSON 파일 직렬화/저장]
    Valid -- "재입력" --> Add
    SaveQuiz --> Menu

    %% 종료
    Menu -- "6. 프로그램 종료" --> Exit((종료))

    %% 스타일 설정
    style Start fill:#f9f,stroke:#333,stroke-width:2px
    style Exit fill:#ccf,stroke:#333,stroke-width:2px
    style Menu fill:#fff4dd,stroke:#d4a017,stroke-width:2px
```
...
flowchart TD
    ...
    %% 노드 정의 (오류 방지를 위해 한 줄에 하나씩 정의)
    Start([🚀 프로그램 시작])
    LoadData{💾 state.json 로드}
    Default[기본 퀴즈 생성]
    Init[QuizGame 객체 초기화]
    Menu{📋 메인 메뉴}
    Play[🎬 퀴즈 풀기]
    Add[➕ 퀴즈 추가]
    History[📜 히스토리 출력]
    ...
    %% 스타일 적용
    class Start,Exit startStop;
    class Init,Play,Add,History process;
    class LoadData,Menu decision;
    class Default,Save1,Save2 storage;

    %% 흐름 연결
    Start --> LoadData
    LoadData -- "파일 없음/손상" --> Default
    LoadData -- "정상 로드" --> Init
    Default --> Init
    Init --> Menu
